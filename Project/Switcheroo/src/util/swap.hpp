...
#ifdef _MSC_VER
[[nodiscard]] inline u16 swap16(const u16 data) noexcept {
    return _byteswap_ushort(data);
}
[[nodiscard]] inline u32 swap32(const u32 data) noexcept {
    return _byteswap_ulong(data);
}
[[nodiscard]] inline u64 swap64(const u64 data) noexcept {
...
public:
    ...
    friend S operator+(const S& p, swapped_t v);
    ...
    // Comparison
    template <typename S, typename T2, typename F2>
    friend bool operator<(const S& p, swapped_t v);
...
using s16_le = AddEndian<i16, LETag>::type;
using s32_le = AddEndian<i32, LETag>::type;
using s64_le = AddEndian<i64, LETag>::type;
...
using s16_be = AddEndian<i16, BETag>::type;
using s32_be = AddEndian<i32, BETag>::type;
using s64_be = AddEndian<i64, BETag>::type;
